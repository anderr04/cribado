...
class Portfolio:
    ...
    def get_positions(self):
        """Devuelve diccionario con todas las posiciones {ticker: {'shares': X, 'average_price': Y}}."""
        positions = {}
        with self._get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute("SELECT ticker, shares, average_price FROM positions")
            for row in cursor.fetchall():
                positions[row[0]] = {"shares": row[1], "average_price": row[2]}
        return positions
    ...
    def buy(self, ticker, price, total_investment):
        ...
        with self._get_connection() as conn:
            ...
            if row:
                ...
                cursor.execute("UPDATE positions SET shares = ?, average_price = ? WHERE ticker = ?", 
                               (new_shares, new_avg_price, ticker))
            else:
                cursor.execute("INSERT INTO positions (ticker, shares, average_price) VALUES (?, ?, ?)", 
                               (ticker, shares, price))
    ...
    def sell(self, ticker, price):
        ...
        with self._get_connection() as conn:
            ...
            if not row:
                print(f"  [INFO] No hay particiones de {ticker} en cartera para vender.")
                return False
        ...
        print(f"  [VENTA] {ticker}: {shares:.4f} shares a ${price:.2f} (Total: ${total_return:.2f}) | PnL: ${profit_loss:.2f} ({profit_pct:.2f}%)")
        return True

    def get_portfolio_summary(self):
        """Devuelve resumen de cartera incluyendo valoraciones base (sin actualizar price por red)."""
        cash = self.get_cash()
